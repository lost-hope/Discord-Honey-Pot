...
client.on(Events.MessageCreate, message => {
    ...
    // Ignore DMs and only process messages from the configured guild/channel.
    ...
    if (message.guild.id === serverid && message.channel.id === channelID) {
       // Ban the user who sent the message           
        message.member.ban({ reason: 'Banned for sending a message in the WLED channel.', deleteMessageSeconds: 60* 60 * 24 * 7 })
            .then(() => {
                // Log metadata only so Message Content intent is not required.
                const logMessage = `Banned ${message.author.tag} for sending a message in the honey pot. Channel ID: "${message.channel.id}" Server: "${message.guild.name}"`;
